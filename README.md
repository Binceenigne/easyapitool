...
- 右侧按每次提交创建一个图片集，每张图片的排队、生成、partial 过程预览、完成或失败状态都会实时更新；过程图仅显示加载动画且不可打开，收到明确完成事件后才开放大图查看。
...
- 最终图片自动保存到 Windows“图片”目录下的 `DJYX_APITOOL` 文件夹，可从结果区直接打开文件夹、将单图另存为其他位置，或在查看器中右键复制完整图片到 Windows 剪贴板。
